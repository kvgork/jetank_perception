...
{
  RCLCPP_INFO(
    logger, "%s - Blur: %.2f, Brightness: %.2f±%.2f, Contrast: %.2f",
...
{
  RCLCPP_INFO(
    logger, "Rectification - Mean epipolar error: %.3f px, Max: %.3f px, Quality: %s",
  ...
  int num_samples)
{
  ...
  for (int i = 0; i < num_samples; ++i) {
    ...
    cv::Rect template_roi(x - template_size / 2, y - template_size / 2, template_size,
      template_size);
...
{
  RCLCPP_INFO(
    logger, "Disparity - Valid: %.1f%% (%d/%d), Range: [%.1f, %.1f], Mean: %.1f±%.1f",
...
DisparityQualityMetrics analyze_disparity_quality(const cv::Mat & disparity)
{
  ...
  if (!valid_values.empty()) {
    ...
    metrics.std_disparity = std::sqrt(
      sq_sum / valid_values.size() -
...
{
  RCLCPP_INFO(
    logger, "%s Point Cloud - Total: %d, Finite: %d (%.1f%%), Outliers: %d (%.1f%%)",
    stage.c_str(), total_points, finite_points, density * 100,
    outliers_detected, outlier_ratio * 100);
  RCLCPP_INFO(
    logger, "  Spatial - Z: [%.3f, %.3f], Mean: (%.3f, %.3f, %.3f), Std: (%.3f, %.3f, %.3f)",
    min_z, max_z, mean_x, mean_y, mean_z, std_x, std_y, std_z);
}

PointCloudQualityMetrics analyze_pointcloud_quality(
...
cv::Mat create_disparity_colored(const cv::Mat & disparity, double min_disp, double max_disp)
{
  ...
  if (range > 0) {
    ...
    cv::normalize(
      disparity_float, disparity_normalized, 0, 255, cv::NORM_MINMAX, CV_8U,
      valid_mask);
  }
